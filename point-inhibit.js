exports.inhibite = (msg) => {
	if(msg.content.length == 1) return true;
	return spamchannel(msg.channel.name) || blockprefix(msg);
}

function blockprefix(message) {
	// While we're not trying to block all bot prefixes
	...
	// this function is served to block most bots on it's own without the use of a config.
