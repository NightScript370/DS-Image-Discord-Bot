...
export default class NintendoSwitchCommand extends Command {
	...
	async exec(message, { images, rating, chubby, forcestretch, internet, funky, pattern }) {
		...
		const base = await loadImage(join(process.cwd(), 'assets', 'images', 'switch', 'switch_case.png'));
		...
		if (internet) {
			let internetImg = await loadImage(join(process.cwd(), 'assets', 'images', 'switch', 'internet_download.png'));
			ctx.drawImage(internetImg, 0, 0, base.width, base.height)
		}

		if (funky) {
			let funkyImg = await loadImage(join(process.cwd(), 'assets', 'images', 'switch', 'funky_mode.png'));
		...
		return message.util.send(global.translate(message.author.lang, "{0}, here's a console game, now its portable!", message.guild ? message.member.displayName : message.author.username), { files: [{ attachment: attachment, name: 'nintendo-switch-boxart.png' }] });
	}
};
