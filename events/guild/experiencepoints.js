	...
	antispam(message) {
		...
		if (message.mentions && message.mentions.members) {
			if(message.mentions.members.size > 11) {
				this.client.moderation.ban(message.member, 'Mention spamming in #'+message.channel.name, mod, message);
        return true;
      }

			if(message.mentions.members.size > 4) {
				this.client.moderation.warn(message.author, 'Mass mentioning', mod, message);
				message.reply("Do not mention spam.");
        return true;
			}
		}
