...
module.exports = class SurprisedPikaPanelCommand extends Command {
	...
	async exec(message, { items }) {
		...
		if (items.length < 1)
			return message.util.send(global.getString(msg.author.lang, "There are not enough arguments to this command. The minimum is {0}.", 1));
		...
		message.util.send(global.getString(msg.author.lang, "Alright {0}, here's your scared pikachu!", message.guild ? message.member.displayName : message.author.username), { files: [{attachment: attachment, name: 'brain.png'}] })
	}
