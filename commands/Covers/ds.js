...
module.exports = class NintendoDSCommand extends Command {
	...
	async exec(message, { images, rating, padding, internet, funky, pattern }) {
		...
		return message.util.send(`${message.guild ? message.member.displayName : message.author.username}, tappity tap!`, { files: [{ attachment: attachment, name: 'Nintendo-DS.png' }] });
	}
};
