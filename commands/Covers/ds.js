...
module.exports = class NintendoDSCommand extends Command {
	...
	async exec(message, { images, rating, padding, internet, funky, pattern }) {
		...
		return message.util.send(global.lang.getString(message.author.lang, "{0}, tappity tap!", message.guild ? message.member.displayName : message.author.username), { files: [{ attachment: attachment, name: 'Nintendo-DS.png' }] });
	}
};
