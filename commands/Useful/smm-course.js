...
module.exports = class CourseCommand extends Command {
  ...
  async handleLevel(msg, ID) {
    try {
      ...
      let SMMCourseEmbed = this.client.util.embed()
      ...
      switch(levelinfo.game_style_raw) {
			  case "sb":
				  SMMCourseEmbed.addInline("Game Style", "Super Mario Bros.")
				                .setColor("#D54B00");
			    break;
		    case "sb3":
			    SMMCourseEmbed.addInline("Game Style", "Super Mario Bros. 3")
                        .setColor("#FAEBD6");
				  break;
			  case "sw":
			    SMMCourseEmbed.addInline("Game Style", "Super Mario World")
			                  .setColor("#01F406");
			    break;
		    case "sbu":
          SMMCourseEmbed.addField("Game Style", "New Super Mario Bros. U")
                        .setColor("#0096C8");
		  }

      SMMCourseEmbed.addInline('World Record', `${levelinfo.world_record.time} by [${levelinfo.world_record.name}](${levelinfo.world_record.user_url})`)
                    .addInline('First Clear', `By [${levelinfo.first_clear.name}](${levelinfo.first_clear.user_url})`)
